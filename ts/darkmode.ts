...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const darkModeToggle: Element | null = document.getElementsByClassName("toggle_dark_mode")[0] ?? null; // Dark Mode Toggle

    // If darkModeToggle is null, go to error page
    if(!darkModeToggle) {
    ...
    function toggleDisplayMode(){
        ...
        const bodyElement: HTMLElement | null = document.body ?? null; // Body
        const icons: NodeListOf<HTMLElement> | null = document.querySelectorAll(".toggle_icon, .link_icon, .normal_icon"); // Icons
        const startContainer: Element | null = document.getElementsByClassName("main_title_container")[0] ?? null; // Start Container
        const bodyContainer: Element | null = document.getElementsByClassName("main_body")[0] ?? null; // Content Container
        const paragraphs: NodeListOf<HTMLElement> | null = document.querySelectorAll(".toggle_white, .toggle_lightblue, .toggle_darkblue"); // Paragraphs

        const toggleIcon: Element | null = document.querySelector(".fa-sun , .fa-moon");
        ...
        for(let i = 0; i < paragraphs.length; i++)
            ...
            else if(paragraphs[i]?.classList.contains("toggle_darkblue"))
                paragraphs[i]?.classList.toggle("font_toggle_darkblue_light_mode");
