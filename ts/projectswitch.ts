import {projectsArray, type projectLocation} from "./projectlist.js"; // Import projectsArray 
    

// Add an event listener when loading up the skills webpage
document.addEventListener("DOMContentLoaded", async function(){

    const scrollableContainer: Element | null = document.getElementsByClassName("scrollable_container")[0] ?? null; // Scrollable container
    const projectsHeadersContainer: Element | null  = document.getElementsByClassName("projectslist_container")[0] ?? null; // Projects Header container
    const projectDetailsContainer: Element | null  = document.getElementsByClassName("projects_container")[0] ?? null; // Projects Details container

    const returnToMain: Element | null = document.getElementsByClassName("return_container")[0] ?? null; // Return to main page element
    const returnToList: Element | null = document.getElementsByClassName("return_projectlist_container")[0] ?? null; // Return to project list element

    // Create variables for projectHeaders for the list of projects to be filled out later.
    let projectHeaders: Element[]  = [];
    ...
    projectHeaders.forEach(async (projectHeader, index) => {
        ...
        const projectHeaderPic: Element | null = projectHeader.getElementsByClassName("projectlist_pic")[0] ?? null;
    ...
    async function loadProjectHeaders(): Promise<void>{
        ...
        for(const project of projectsArray){   
            ...
            const projectHeader: string = (isLightMode) 
                ?                  
                `
            ...
            projectHeaders.push(projectsHeadersContainer?.lastElementChild as Element ?? null);
    ...
    async function loadProjectDetails(projectInfo: any): Promise<void>{

        // Obtain project description elements
        const title: Element | null = document.getElementsByClassName("project_title")[0] ?? null;
        const img: HTMLImageElement | null = document.getElementsByClassName("project_pic")[0] as HTMLImageElement ?? null;
        const description: Element | null = document.getElementsByClassName("description")[0] ?? null;
        const link: HTMLLinkElement | null = document.getElementsByClassName("button_link")[0] as HTMLLinkElement ?? null;

        const technologyContainer: Element | null = document.getElementsByClassName("technologies_container")[0] ?? null; // Technologies container
        const currentTechnologies: HTMLCollectionOf<Element> | null = document.getElementsByClassName("technology_element"); // Reset the current technologies of the project
        ...
        for(let i = projectInfo.technologies.length - 1; i >= 0; i--)
            ...
            `<p class = "content technology_element toggle_lightblue"> ${projectInfo.technologies[i]} </p>`);
    ...
    async function addLinkIcon(linkElement: Element, projectLocation: projectLocation): Promise<void>{
    ...
    async function toggleProjectList(): Promise<void>{
